  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-8">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#0B3A53] to-[#146C86] text-white p-6 flex items-center justify-between">
          <div>
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 block">
              Add Traveler Photographs (Mock Upload)
            </label>
            ...
            {photos.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 pt-2">
                {photos.map((pic, idx) => (
                  <div key={idx} className="relative group w-16 h-16 rounded-xl overflow-hidden border border-slate-300">
                    <img src={pic} alt="Upload preview" className="w-full h-full object-cover" />
                    <button
                      type="button"
                      onClick={() => handleRemovePhoto(idx)}
                      className="absolute top-1 right-1 bg-black/70 hover:bg-rose-600 text-white p-0.5 rounded-full cursor-pointer"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Scenic, Wildlife, Photography, Culture"
              className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-[#16A6A1] focus:outline-none"
            />
          </div>
